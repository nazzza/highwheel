...
public class OrphanAnalyser<V, E> {
  ...
  public List<V> findsDeadMethods(DirectedGraph<V, E> methodCalls,
      List<V> entryPoints) {
    // List<V> orphans = Collections.emptyList();
            ...
            if (areThereAnyEdges(methodCalls)) {
              ...
              for (V entryPoint : entryPoints) {
                if (methodCalls.outDegree(entryPoint) == 0) {
                  System.out.println("entry point has no out edges");
              ...
              // got through all empty points and remove them from the graph
              for (V entryPoint : entryPoints) {
                methodCalls.removeVertex(entryPoint);
              }
              // all Vs left are orphans
              orphans.addAll(methodCalls.getVertices());
            }
          }
        }
  ...
  private boolean isEmptyGraph(DirectedGraph<V, E> methodCalls) {
  ...
  private boolean areAllMethodsEntryPoints(DirectedGraph<V, E> methodCalls,
      List<V> entryPoints) {
  ...
  private boolean areThereAnyEntryPoints(List<V> entryPoints) {
  ...
  private boolean areThereAnyEdges(DirectedGraph<V, E> methodCalls) {
  ...
  private boolean areEntryPointsInsideAGraph(DirectedGraph<V, E> methodCalls,
      List<V> entryPoints) {
    for (V entryPoint : entryPoints) {
